...
public class EfficientDocument extends Document {
	...
	private void processText() {
		...
		// List<String> tokens_sent = getTokens("[^?.!]+");
...
//		}
		...
		for (int i = 0; i < tokens.size(); i++) {
			...
			if (i == tokens.size() - 1 && !tokens.get(i).endsWith("!") && !tokens.get(i).endsWith("?")
					&& !tokens.get(i).endsWith(".") && !tokens.get(i).endsWith(" ") && tokens.get(i) != "") {
				numSentences++;
			}
